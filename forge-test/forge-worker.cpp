...
int main() {
    ...
    while (true) {
        ...
        if (!recv_all(
            ...
            std::cerr << "Failed receiving filename length\n";
            close(client_fd);
            continue;
        }
        ...
        if (!recv_all(
            ...
            std::cerr << "Failed receiving filename\n";
            close(client_fd);
            continue;
        }

        // Only keep the filename, not any client path.
        filename =
        ...
        if (!recv_all(
            ...
            std::cerr << "Failed receiving file size\n";
            close(client_fd);
            continue;
        }
        ...
        if (file_size > 0 &&
            ...
            std::cerr << "Failed receiving file contents\n";
            close(client_fd);
            continue;
        }
        ...
        std::filesystem::path source_path =
            std::filesystem::path("received") / filename;
        ...
        if (!source_output) {
            std::cerr << "Failed creating source file\n";
            close(client_fd);
            continue;
        }
        ...
        // -------------------------------------------------
        // Tell client whether compilation succeeded
        // -------------------------------------------------

        uint32_t status =
            (compile_result == 0) ? 1 : 0;
        ...
        if (!send_all(
            ...
            std::cerr << "Failed sending status\n";
            close(client_fd);
            continue;
        }
        ...
        if (!send_all(
            ...
            std::cerr << "Failed sending object filename length\n";
            close(client_fd);
            continue;
        }
        ...
        if (!send_all(
            ...
            std::cerr << "Failed sending object filename\n";
            close(client_fd);
            continue;
        }
        ...
        if (!send_all(
            ...
            std::cerr << "Failed sending object size\n";
            close(client_fd);
            continue;
        }
        ...
        if (object_size > 0 &&
            ...
            std::cerr << "Failed sending object file\n";
            close(client_fd);
            continue;
        }
